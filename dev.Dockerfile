...
COPY --chown=$PLUGIN_BUILD_UID:$PLUGIN_BUILD_GID ./plugins/adl_fieldclimate_plugin/requirements/dev.txt /tmp/plugin-dev-requirements.txt
RUN . /adl/venv/bin/activate && pip3 install -r /tmp/plugin-dev-requirements.txt

COPY --chown=$PLUGIN_BUILD_UID:$PLUGIN_BUILD_GID ./plugins/adl_fieldclimate_plugin/ $ADL_PLUGIN_DIR/adl_fieldclimate_plugin/
RUN . /adl/venv/bin/activate && /adl/plugins/install_plugin.sh --folder $ADL_PLUGIN_DIR/adl_fieldclimate_plugin --dev
