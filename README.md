...
ADL Plugin fro collection observation data from FieldClimate API
...
The ADL FieldClimate Plugin is a module intended to be installed in an [ADL](https://github.com/wmo-raf/adl)
...
### Install ADL FieldClimate Plugin

The `dev.Dockerfile` file uses the `adl` image as a base image. The `ADL FieldClimate Plugin` is
...
```bash
git clone https://github.com/wmo-raf/adl-fieldclimate-plugin.git
cd adl-fieldclimate-plugin
```
